	...
	private void loadMessageTypeUIExtensions() {
		...
		for (int i = 0; i < extensions.length; i++) {
			...
			try {
				...
				participant.setProviderId((String) extnData.getAttribute(EXTENSION_PROVIDERID));
				participant.setType((String) extnData.getAttribute(MESSAGE_TYPE));
	...
	private void loadMessageTypeExtensions() {
		...
		for (int i = 0; i < extensions.length; i++) {
			...
			try {
				...
				participant.setProviderId((String) extnData.getAttribute(EXTENSION_PROVIDERID));
				participant.setType((String) extnData.getAttribute(MESSAGE_TYPE));
				participant.setSupportedFileExtension((String) extnData.getAttribute(MESSAGE_SUPPORTED_FILE_EXTENSION));
