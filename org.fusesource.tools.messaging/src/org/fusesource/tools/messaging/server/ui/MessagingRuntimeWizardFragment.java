...
public class MessagingRuntimeWizardFragment extends WizardFragment {
    ...
    protected void createControl(Composite parent) {
        ...
        for (String string : keys) {
            String key = string;
            String value = connectionParams.get(key);
            ...
            if (dynamicComp != null && !dynamicComp.isDisposed()) {
                ...
                for (Control control : controls) {
                    if (control instanceof Text) {
                        if (((Text) control).getText() != null && ((Text) control).getText().length() <= 0) {
                            isValid = false;
                        }
                    }
                }
                ...
                try {
                    ...
                    for (Control control : controls) {
                        if (control.getData() != null) {
                            for (String string : keys) {
                                key = string;
                                if (control.getData().toString().equals(key)) {
                                    connectionProps.put(key, ((Text) control).getText());
                                }
                            }
                        }
                    }
