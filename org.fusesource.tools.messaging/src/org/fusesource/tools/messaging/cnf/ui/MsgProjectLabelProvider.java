...
public class MsgProjectLabelProvider extends FontBasedLabelProvider implements
	...
	public Font getFont(Object element) {
		if (element instanceof ListenerComponent) {
			IListener listener = ((ListenerComponent) element).getListener();
			IMessagesManager messagesManager = listener
					.getMessagesManager();
			if (messagesManager == null)
				return MsgProjectChangeHandler.DEFAULT_FONT;
			int unreadCount = messagesManager
					.getMessagesCount(IMessageChangeListener.MESSAGE_UNREAD);
			if (unreadCount > 0)
	...
	private String getListenerDisplayText(IListener listener) {
		...
		int unreadCount = listener.getMessagesManager().getMessagesCount(
				IMessageChangeListener.MESSAGE_UNREAD);
		if (unreadCount <= 0)
			return displayText.toString();
	...
	/**
	 * shows the type & name of the selected project viewer element in the
	 * status bar
	 */
	public String getDescription(Object anElement) {
		if (anElement instanceof IProject) {
			IProject project = (IProject) anElement;
			IServer deployedServer = MessagingServersUtil
					.getDeployedServer(project);
			if (deployedServer != null)
				return project.getName() + " is deployed in "
						+ deployedServer.getName();
		} else if (anElement instanceof SenderComponent) {
			ISender sender = ((SenderComponent) anElement).getSender();
			return getType(sender.getDestination()) + ": "
					+ getDefaultText(sender.getDestination());
		} else if (anElement instanceof ListenerComponent) {
			IListener listener = ((ListenerComponent) anElement).getListener();
			return getType(listener.getDestination()) + ": "
					+ getDefaultText(listener.getDestination());
