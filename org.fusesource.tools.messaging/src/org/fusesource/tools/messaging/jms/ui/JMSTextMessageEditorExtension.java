...
public class JMSTextMessageEditorExtension extends JMSSimpleMessageEditorExtension {
	...
	public void createBody(Composite parent, EditingDomain editingDomain, Message messageModel) {
		...
		final URLChooser chooser = new URLChooser(parent, lst, URLChooser.STYLE_NONE);
	...
	protected void setContentToModel(String anyType) {
		...
		if (body == null) {
			...
			Command create = CreateChildCommand.create(editingDomain, messageModel, commandParameter, new StructuredSelection(messageModel).toList());
	...
	protected void setContentToModel(Body body, String contentStr) {
		try {
			...
			if (content instanceof AnyType) {
				...
				create = SetCommand.create(editingDomain, body, MessagePackage.eINSTANCE.getBody_Content(), contentData, 0);
				...
				create = CreateChildCommand.create(editingDomain, body, commandParameter, new StructuredSelection(body).toList());
			}
			Command createCmd = SetCommand.create(editingDomain, body, MessagePackage.eINSTANCE.getBody_UseFileRef(), false, 0);
	...
	/**
	 * 
	 * @param text this is the file url to be used
	 * @param useText true if text is not null | not to be used 
	 * @param useFileRef whether to use the file ref as body or not 
	 */
	protected void modifyBodySource(String text, boolean useText, boolean useFileRef) {
		...
		try {
			...
			List<Command> commandList = new ArrayList<Command>();
			if (useText) {
				Command create = SetCommand.create(editingDomain, body, MessagePackage.eINSTANCE.getBody_FileRef(), text, 0);
				commandList.add(create);
			}
			Command createCmd = SetCommand.create(editingDomain, body, MessagePackage.eINSTANCE.getBody_UseFileRef(), useFileRef, 0);
			commandList.add(createCmd);
			CompoundCommand compoundCommand = new CompoundCommand(commandList);
			editingDomain.getCommandStack().execute(compoundCommand);
		} catch (Throwable t) {
