...
public class MessagesMasterBlock extends MasterDetailsBlock implements IDetailsPageProvider {
	...
	private void addDeleteAllAction(IToolBarManager toolBarManager) {
		deleteAllAction = new Action("Delete All", Action.AS_PUSH_BUTTON) {
			public void run() {
				clearMessages();
				updateActions();
			}
	...
	private void addDeleteAction(IToolBarManager toolBarManager) {
		deleteAction = new Action("Delete", Action.AS_PUSH_BUTTON) {
			public void run() {
				deleteMessage();
				updateActions();
			}
	...
	private void addSaveAsAction(IToolBarManager toolBarManager) {
		saveAsAction = new Action("Save As", Action.AS_PUSH_BUTTON) {
			public void run() {
				saveMessage();
				updateActions();
			}
	...
	protected void clearMessages() {
		boolean answer = MessageDialog.openQuestion(Display.getCurrent().getActiveShell(), "Delete All", "Are you sure you want to delete all messages from the listener?");
		if (!answer)
			return;
	...
	protected void deleteMessage() {
		...
		if (messageEvent == null)
			return;
		boolean answer = MessageDialog.openQuestion(Display.getCurrent().getActiveShell(), "Delete", "Are you sure you want to delete the message from the listener?");
		if (!answer)
			return;
			...
			try {
				...
				if (dlg.open() == Window.OK) {
					...
					if (path != null) {
						...
						IMessageType messageType = MessageExtensionsMgr.getInstance().getMessageTypeExtension(messageEvent
								.getMessage(), id);
						if (messageType == null) {
	...
	public Object getPageKey(Object arg0) {
		...
		if(keyObject==null)
		keyObject = MessageExtensionsMgr.getInstance().getMessageTypeUIExtension(messageEvent.getMessage(), JMSConstants.DEFAULT_JMS_PROVIDER);
		return keyObject;
	}
