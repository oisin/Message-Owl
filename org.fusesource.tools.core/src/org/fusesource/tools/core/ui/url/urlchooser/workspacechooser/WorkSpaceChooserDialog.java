...
public class WorkSpaceChooserDialog extends SelectionDialog {
	 ...
	 * @param lp
	 *            Add your own LabelProvider <code>null</code> to display a
	 *            default message
	 */
	public WorkSpaceChooserDialog(Shell parentShell, IResource initialRoot,
			String title, boolean allowNewContainerName, String message,
			IContentProvider cp, ILabelProvider lp) {
		this(parentShell, initialRoot, null, title, allowNewContainerName,
				message);
		this.cp = cp;
		this.lp = lp;
	}
	...
	public WorkSpaceChooserDialog(Shell parentShell,
			IResource initialSelection, Map filter, String title,
	...
	public WorkSpaceChooserDialog(Shell activeShell,
			IResource currentSelectedResource, URLChooserFilter filter2,
			String title, boolean allowNew, String msg,
			IFolder rootFolder) {
	...
	private String getSelectedFilterExtension() {
		String selectedExtension = selectedFilter.substring(selectedFilter
				.indexOf('.') + 1, selectedFilter.length());
	...
	protected void validate() {
		...
		String errorMsg;
		if (ALL_DIRECTORIES.equals(extensionsCombo.getItem(extensionsCombo
				.getSelectionIndex()))) {
			errorMsg = null;
		} else {
	...
	protected WorkSpaceSelectionGroup createWorkspaceGroup(Composite area,
			Listener listener) {
		List filters = getFiltersOnPath();
		return new WorkSpaceSelectionGroup(area, listener,
	...
	protected IPath getContainerFullPath() {
		...
		if (fullPath != null) {
			...
			if (folders == null || (folders.length > 0 && !folders[0].exists())) {
				if (fullPath != null
						&& !extension.equals("*")
						&& (fullPath.getFileExtension() == null || !(fullPath
								.getFileExtension().equalsIgnoreCase(extension)))) {
	...
	public void refreshTree() {
		String item = extensionsCombo.getItem(extensionsCombo
				.getSelectionIndex());
	...
	protected static class CustomViewFilter extends ViewerFilter {
		...
		protected boolean qualifiesFilter(IFile f) {
			if (extensionSelected != null) {
				...
				if (dot < 0) // Invalid filter selected
					return true;
